...
Alternative to get_pmf.py for shorter (100 ns) trajectories.

Take the WHAM metadata produced by extract_metadata.py for the *last 50 ns*
of a 100 ns umbrella-sampling trajectory (section51..section100, assuming
1 ns per section*.dcd chunk) and write
    <out-dir>/pmf_s1.dat   per-solute WHAM PMF (z, PMF, ...)
    <out-dir>/pmf_s2.dat
    <out-dir>/pmf-us-<analog>.dat
        |z|(A)   PMF(kcal/mol)   SE(kcal/mol)   n_leaflets

Unlike get_pmf.py (which averages/estimates the error across three 50 ns
blocks: section101-150, section151-200, section201-250), this script
estimates the average and standard error from the difference between the
two leaflets (solute 1, +z leaflet vs. solute 2, -z leaflet), using a
single 50 ns window of data. This is meant for analogs that were only run
for 100 ns, where three independent 50 ns blocks are not available.
...
standard error between the two leaflet estimates, i.e. std/sqrt(n).
...
Usage (see also run2.sh):
    extract_metadata.py --analog <a> --runs-dir ../runs --out-dir pmf2/<a> \\
        --equil-ps 0 --min-section 51 --max-section 100
    get_pmf2.py --analog <a> --in-dir pmf2/<a> --out-dir pmf2/<a>
"""
...
def main() -> int:
    ...
    ap.add_argument("--in-dir",   required=True, type=Path,
                    help="directory with metadata_s1.dat and metadata_s2.dat "
                         "(e.g. produced with --min-section 51 --max-section 100)")
